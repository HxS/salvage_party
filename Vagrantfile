...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision 'shell', inline: <<-SHELL
    if [ ! -e /opt/bin/docker-compose ]; then
      ...
      mkdir -p /opt/bin
      curl -sL https://github.com/docker/compose/releases/download/1.8.0/docker-compose-`uname -s`-`uname -m` > /opt/bin/docker-compose
  ...
  (1..$num_instances).each do |i|
    config.vm.define vm_name = "%s-%02d" % [$instance_name_prefix, i] do |config|
      ...
      $shared_folders.each_with_index do |(host_folder, guest_folder), index|
        config.vm.synced_folder host_folder.to_s, guest_folder.to_s, id: "salpa-share%02d" % index, nfs: true, mount_options: ['nolock,vers=3,udp']
      end
